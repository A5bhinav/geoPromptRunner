...
that finding did not reproduce.) It was briefly lowered to K=3 on 2026-08-01 as a
cost/breadth trade and **restored to K=5 the same day**: a 60% worst-brand floor means a
3-run reading can simply be wrong, and more queries do not repair a noisy per-query
reading. See ``settings.DEFAULT_RUNS_PER_QUERY`` for the full reasoning — the fallback
band below inherits whatever that default is. Local is noisier still — SE

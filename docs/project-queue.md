...
|---|---|---|---|
| **1 — Engine-answer judge** | LLM reading of engine answers (present / prominence / framing + accuracy flags) | `src/pipeline/judge_metrics.py`, gold sets `data/oura_gold.json` / `data/fort_gold.json`; `scripts/run_calibration.py` | ✅ **Done** — re-confirmed 2026-06-28 under the B2C/consumer judge prompt (Sonnet, temp 0, dedup, gold sets web-verified). Agreement: present 96% · prominence 88% · framing 93% · **flag recall 95%** (was 100% pre-B2C — one of 21 gold flags now missed) · binary flag agreement 89%. **Flag precision ~~42%~~ → 80% (recall held 95%, 5 FP / 20 TP / 1 FN) — SOLVED 2026-06-28 via the adversarial verifier (`JUDGE_VERIFY`), see queue #9. The verifier is opt-in; `JUDGE_VERIFY=1` is now set in `.env` so any outreach/teaser run gets the 80% path. Base single-Sonnet judge (verifier off) still over-flags at ~42% — never send flags from a run without the verifier.** |

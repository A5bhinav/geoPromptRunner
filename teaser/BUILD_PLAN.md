...
| 4 | **Niche — SUPERSEDED 2026-07-31; was "RESOLVED: B2C consumer"** (the SMB pivot added a `local_service` ICP, Phases 0–5 complete, and local-service is the live GTM; the consumer path stays live alongside it — see `docs/smb-pivot-build-plan.md` §0.6) | The GTM is **B2C consumer startups** (Berkeley/SV), matching the platform's README niche + query examples. Engine is category-agnostic; keep query-set generation + fact-sheet examples tuned for **B2C consumer**. Validate the first few teasers on real consumer prospects before volume. |
